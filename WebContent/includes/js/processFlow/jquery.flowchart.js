...
// "colorize" the widget name
 ...
 this.objs.layers.operators.on('dblclick', '.flowchart-operator', function (e) {
              /*  if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
                ...
                if($(this).data('operator_id').indexOf("process")>-1){
                showSourceModal();
            }
            if($(this).data('operator_id').indexOf("map")>-1){
                showMapModal();
            }
            if($(this).data('operator_id').indexOf("target")>-1){
                showTargetMolal();
            }
            });
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator_title="";
            if(infos.processId!=undefined){
                 $operator_title = $('<div class="flowchart-operator-title sourceNode"></div>');
            }else if(infos.mapId!=undefined){
                 $operator_title = $('<div class="flowchart-operator-title mapNode"></div>');
            }else{
                $operator_title = $('<div class="flowchart-operator-title targetNode"></div>');
            }
        ...
        selectOperator: function (operatorId) {
           ...
           // alert("Selected "+operatorId);
            //alert("Selected= "+JSON.stringify(this.data.operators[operatorId].internal.properties, null, 2));
            ...
            var data=JSON.stringify(this.data.operators[operatorId].internal.properties, null, 2);
          // alert(JSON.stringify(this.data.operators[operatorId], null, 2));
        ...
        _deleteLink: function (linkId, forced) {
            ...
            //linkData.internal.els.overallGroup.remove();   //  for IE
