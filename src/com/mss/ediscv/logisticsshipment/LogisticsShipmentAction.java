...
public class LogisticsShipmentAction extends ActionSupport implements ServletRequestAware {
    ...
    private static Logger logger = LogManager.getLogger(LogisticsShipmentAction.class.getName());
        ...
        try {
            if (httpServletRequest.getSession(false).getAttribute(AppConstants.SES_USER_NAME) != null) {
                ...
                if (AuthorizationManager.getInstance().isAuthorizedUser("L_SHIPMENT", userRoleId)) {
                    ...
                    setCorrelationList(DataSourceDataProvider.getInstance().getCorrelationNames(3, 2));
                    setDocTypeList(DataSourceDataProvider.getInstance().getDocumentTypeList(3, 2));
    ...
    public String getShipmentSearchList() throws Exception {
        System.out.println("getShipmentSearchList start time::"+DateUtility.getInstance().getCurrentDB2Timestamp());
        setResultType(LOGIN);
        try {
            if (httpServletRequest.getSession(false).getAttribute(AppConstants.SES_USER_NAME) != null) {
                setResultType("accessFailed");
             ...
             httpServletRequest.getSession(false).setAttribute("exceptionMessage", exception.getMessage());
            LoggerUtility.log(logger, "Exception occurred in getShipmentSearchList method:: " + exception.getMessage(), Level.ERROR, exception.getCause());
        }
        System.out.println("getShipmentSearchList end time::"+DateUtility.getInstance().getCurrentDB2Timestamp());
        return getResultType();
    }
